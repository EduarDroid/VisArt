...
//</editor-fold>
   ...
   Mat mImgEscalaGrises;// = new Mat();
    private void PreProcesarImagen() throws IOException {
        ...
        */        
        //1. REDIMENSION
        //this.mImagenReal=Scalar.all(50);
        /*int beta=50;//1-3
        double alpha=1;//1-100
        Mat new_image= new Mat();
        
       
        for( int y = 0; y < this.mImagenReal.rows(); y++ )
        { for( int x = 0; x < this.mImagenReal.cols(); x++ )
             { for( int c = 0; c < 3; c++ )
                {
                    new_image.at<Vec3b>(y,x)[c] =
                    saturate_cast<uchar>( alpha*( this.mImagenReal.at<Vec3b>(y,x)[c] ) 
                                        + beta );
                }
        }
        }
        
        this.mImagenReal=new_image;*/
...
// = new Mat();
    protected void SegmentarImagen() throws IOException {
        ...
        */
        ...
        this.mImgDilatar = new Mat();
        
        Imgproc.dilate(mImgErode, this.mImgDilatar,mElementoDilatacion);
       
        //Imgproc.floodFill(this.mImgDilatar, mImagenHSV, seedPoint, maxVerde);
        //this.mImgDilatar= 
        //MOSTRAR RESULTADOS
        ivErosion.setImage(this.mat2Image(mImgErode));
        ivErosion.setPreserveRatio(true);
        ivDilatar.setImage(this.mat2Image(this.mImgDilatar));
	...
	int maxRadius = 250;//18
        ...
        /*this.mImgEscalaGrises= new Mat();
        Mat mImg= new Mat(
            // draw the circle center);
        Imgproc.cvtColor(this.mImgDilatar, mImg, Imgproc.COLOR_GRAY2RGB);        
        Imgproc.cvtColor(mImg, this.mImgEscalaGrises, Imgproc.COLOR_RGB2GRAY);

        Imgcodecs.imwrite(strRutaResources+"img/grises.jpg",this.mImgEscalaGrises); */
        
        Imgproc.HoughCircles(this.lstCanalesHSV.get(2) /*this.mImgEscalaGrises*/
                            , circles,Imgproc.CV_HOUGH_GRADIENT,1
                            , 200, 150, 30 //150.30
                            , minRadius
                            , maxRadius);//this.mImagenBinarizada
